...
def acquista_biglietti(username, concerti=None):
    ...
    for idx, concerto in enumerate(concerti):
        table.add_row(str(idx + 1), concerto['nome'], concerto['data'], concerto['artista_nome'], concerto['location_nome'])
    ...
    db.concerti.update_one(
        ...
        {"$set": {"settori.$.posti_disponibili": settore_scelto['posti_disponibili'] - quantita}}
    )
    ...
    artista_utente = db.utenti.find_one({"username": concerto_scelto['artista_nome']})
    
    if artista_utente:
        aggiungi_saldo(concerto_scelto['artista_nome'], costo_totale)
    else:
        hashed_password = bcrypt.hashpw("123".encode('utf-8'), bcrypt.gensalt())
        nuovo_artista_utente = {
            "username": concerto_scelto['artista_nome'],
            "password": hashed_password,
            "saldo": costo_totale,
            "tipo": "artista"
        }
        db.utenti.insert_one(nuovo_artista_utente)
        console.print(f"[green]Creato nuovo account per l'artista {concerto_scelto['artista_nome']} con saldo iniziale di {costo_totale}€ e password predefinita '123'[/green]")
